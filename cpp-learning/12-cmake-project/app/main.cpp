...
"\n  └─────────────────────────────────────┴───────────────────────────────────┘"; };
	...
	//D6
	std::vector<std::shared_ptr<Sensor>> v1;
	auto t1 = std::make_shared<TemperatureSensor>("温度计1",36);
	auto t2 = std::make_shared<TemperatureSensor>("温度计2",34);
	...
	for(auto& ptr : v1)
	{
		std::cout<<ptr->name()<<"->"<<ptr->read()<<std::endl;
	}
